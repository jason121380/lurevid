...
使用者貼上 TikTok／Instagram Reels 連結（或上傳影片檔），系統下載影片、抽影格、轉錄、AI 分析 → 拆結構 → 改編 → 9 格分鏡圖 → 合併成單張 3x3 參考圖 → Seedance 生成影片。

## 兩個進程

- **Web**：`npm run dev`（或 `npm run start`）。提供 UI 與 API，把任務丟進 Redis queue。
- **Worker**：`npm run worker`。處理 `full / source / transcribe / frames / analyze / structure / adapt / storyboard / mergeStoryboard / video` 等 job（見 `scripts/worker.ts`）。建立專案／上傳影片會送 `full`。
...
- 平台下載：yt-dlp 在 Docker 用 nightly（`YTDLP_CHANNEL`）；來源 host allowlist 在 `lib/transcribe.ts` 的 `PLATFORMS`（TikTok、YouTube、IG Reels/限時動態、FB 限時動態），改平台要連同 `app/page.tsx` 的對應檢查一起改。限時動態需要 `YTDLP_COOKIES`（見 `lib/ytdlp.ts`）。另支援直接上傳影片檔（`app/api/projects/upload/route.ts`）。429 是機房 IP 被平台擋，非程式問題。
...
端到端：註冊 → 登入 → 建專案 → 走完八個步驟（下載／轉錄／影格／分析／改編／分鏡／合併分鏡／生成影片）→ 下載成品。並驗證：未登入存取 API 被擋、非擁有者拿不到他人專案、非管理員看不到 `/settings`。

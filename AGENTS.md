...
- 平台下載：yt-dlp 在 Docker 用 nightly（`YTDLP_CHANNEL`）；來源 host allowlist 在 `lib/transcribe.ts` 的 `PLATFORMS`（目前只有 TikTok 與 IG Reels），改平台要連同 `app/page.tsx` 的對應檢查一起改。`YTDLP_COOKIES` 為選填（見 `lib/ytdlp.ts`）。另支援直接上傳影片檔（`app/api/projects/upload/route.ts`）。429 是機房 IP 被平台擋，非程式問題。

...
- `/health` 有一列「Cobalt 下載服務」，顯示 Worker 每 60 秒實測的連線結果（已連線／未設定／連不上）。
  `COBALT_API_URL` 只在 Worker，所以只有 Worker 問得到；沒有這一列的話，環境變數打錯只會每次安靜地回退 yt-dlp，畫面上看不出來。
...
Worker 會並行產生分鏡圖，可用以下變數調整並行上限（預設值已適合大多數情境）：

...
短影音分析創作平台：使用者貼上一支 TikTok 連結，系統會下載影片、抽取影格分析畫面分鏡，
...
- 支援來源：TikTok（`lib/transcribe.ts` 以 allowlist 驗證）。
...
1. 使用者貼上 TikTok 連結，Web 建立 project，worker 開始 `analyze` job。
...
> ⚠️ 從機房 IP（Zeabur）抓 TikTok 影片仍可能被平台阻擋。`analyze` 抓取失敗時 project 會進入 `FAILED`，
> 請確認影片為公開連結，或改用較穩定的網路/代理後重試。

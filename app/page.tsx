...
export default function HomePage() {
  ...
  return (
    ...
    <div className="min-h-screen bg-[var(--warm-white)]">
      <div className="flex h-[60px] items-center justify-between border-b border-[var(--border)] bg-white px-6 shadow-[0_2px_8px_rgba(0,0,0,0.03)]">
        <h1 className="text-base font-extrabold">Seedance Studio</h1>
      ...
      <div className="grid gap-4 p-4 lg:grid-cols-[minmax(0,1fr)_360px] lg:p-6">
        <section className="space-y-4">
          <div className="card p-4">
            <div className="mb-3 flex items-center gap-2">
              ...
              <h2 className="text-sm font-extrabold">輸入想法</h2>
            </div>
            <textarea
            ...
            {scenes.length === 0 ? (
              <div className="card col-span-full grid min-h-[360px] place-items-center p-8 text-center">
                <div>
                  ...
                  <h3 className="font-extrabold">等待分鏡</h3>
              ...
              scenes.map((scene, index) => (
                <article key={scene.sceneNumber} className="card p-3">
                  ...
                  <input
                    className="mb-2 h-9 w-full rounded-lg border border-[var(--border)] px-2 text-sm font-bold"
                    value={scene.title}
                    onChange={(event) => {
                      const next = [...scenes];
        ...
        <aside className="card h-fit p-4 lg:sticky lg:top-6">
          <div className="mb-4">
            <p className="text-[11px] font-bold uppercase text-orange">Production</p>
            <h2 className="mt-1 text-lg font-extrabold">完整影片生成</h2>
          ...
          <div className="space-y-2 rounded-xl bg-[var(--warm-white)] p-3 text-sm">
            <div className="flex justify-between"><span>分鏡數</span><strong>{scenes.length}/9</strong></div>
            <div className="flex justify-between"><span>比例</span><strong>{ratio}</strong></div>
            <div className="flex justify-between"><span>解析度</span><strong>{resolution}</strong></div>
            <div className="flex justify-between"><span>總秒數</span><strong>{scenes.length ? scenes.length * duration : 0}s</strong></div>
          </div>

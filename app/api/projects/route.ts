...
function toCreateProjectError(error: unknown) {
  if (error instanceof z.ZodError) {
    ...
    return {
      message: first?.path[0] === "title" ? first.message : "請貼上有效的 YouTube、TikTok 或 IG Reels 連結",
      status: 400
    };
  }
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!isSupportedSourceUrl(body.sourceUrl)) {
      return NextResponse.json({ error: "目前只接受 YouTube、TikTok 或 IG Reels 連結" }, { status: 400 });
    }
